...
const {
  ...
  getAllStudentsX,
  getAllStudentRecordsByIdTypeAndDate,
}: {
  getAllStudentRecordsById: any;
  getAllStudentsX: any;
...
recordRouter.get("/", async (req: any, res: any) => {
  ...
  const allStudents = await getAllStudentsX();
  let allRecords = [];
  for (let id = 1; id <= allStudents.length; id++) {
    const studentRecords = await getAllStudentRecordsById(id);
